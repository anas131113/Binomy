/**
 * BINOMY — chat.js
 * Messagerie en temps quasi-réel via polling Fetch API
 */
...
async function loadMatches() {
    ...
    try {
        const res  = await fetch('/api/matches/get_matches.php');
        const data = await res.json();

        if (!data.success || !data.data.length) {
            list.innerHTML = '<p style="padding:16px;color:var(--text-muted);text-align:center">Aucun match pour l\'instant</p>';
            return;
        }

        list.innerHTML = data.data.map(m => `
            <div class="chat-list-item" onclick="openMatchChat(${m.match_id}, ${m.partner_id}, '${escHtml(m.partner_name)}')">
...
async function loadMessages() {
    ...
    try {
        const res  = await fetch(`/api/messages/get_conversation.php?${params}`);
        const data = await res.json();
        ...
        area.innerHTML = data.data.map(m => `
            <div>
                <div class="chat-bubble ${m.sender_id == currentUserId ? 'sent' : 'recv'}">
                    ${escHtml(m.content)}
                </div>
...
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('chat-list')) loadMatches();
